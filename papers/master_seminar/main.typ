...
#show: project.with(
  ...
  authors: (
    (
      name-ja: "船瀬・五十里研究室 修士2年　37-256364 高本英熙（2026/6/19）",
      affiliation-ja: "",
      presenting: true,
    ),
  ),
...
と表せる．$L = 1$ m，$Delta x = 100$〜$200$ µm程度であれば，$Delta theta_("LOS")$ は $100$〜$200$ µrad程度となる．これは，小型衛星光通信で想定されるビーム拡がり角や捕捉センサ視野，姿勢決定由来のLOS誤差と同程度であり，粗捕捉のuncertainty budgetに対して有意な寄与を持つ．
...
以上を踏まえ，本研究では熱変形由来の時間変動LOSバイアスを，軌道位相や熱状態から部分的に予測可能な誤差成分として扱う．そして，その予測値を粗捕捉時のscan center補正に用い，捕捉時間や必要scan areaに与える効果を評価する．

= 提案手法
== 基本方針
本研究の目的は，熱ひずみによる時間変動LOSバイアスを予測し，粗捕捉時のscan centerを補正することで，不確定領域と捕捉時間を低減することである．従来の構造設計による熱変形低減は重要であるが，設計段階で全ての運用条件を吸収することは難しい．そこで本研究では，リモートセンシング分野で検討されてきた熱変形LOS補正の考え方を光通信粗捕捉に応用し，熱ひずみを単なる未知外乱ではなく，軌道位相・熱環境・運用モードから部分的に予測可能な低周波バイアスとして扱う．
...
= 評価計画
== シミュレーション構成
提案手法の有効性は，熱ひずみ由来LOSバイアスを含む粗捕捉シミュレーションにより評価する．評価対象は，補正なし，static bias correction，feedforward correction，feedforward + adaptive correctionの4ケースとする．補正なしは熱ひずみ由来バイアスをそのまま初期指向誤差として扱う基準ケースであり，static bias correctionは時間平均的なLOSオフセットのみを補正する比較対象である．feedforward correctionでは，衛星筐体・光学系の構造と熱状態に基づくphysical thermal deformation modelを主候補として，軌道位相や温度情報から予測したLOSバイアスを用いる．feedforward + adaptive correctionでは，この事前予測に加えて観測残差に基づくモデル更新を行う．
...
#figure(
  table(
    ...
    [Feedforward], [Physical thermal deformation model等で予測したLOSバイアスによりscan centerを補正], [事前熱モデルの効果評価],
...
今後の課題は三つある．第一に，Thermal Desktop / Femap解析により，代表的な衛星構造と軌道条件に対するLOSバイアス参照時系列を作成する必要がある．第二に，physical thermal deformation model，Fourier model，orbital geometry model，thermal sensor modelなどの軽量補正モデルを比較し，軌道上実装に適した入力とモデル次数を決定する必要がある．第三に，PAT・姿勢・熱センサ情報から熱状態と相関する低周波残差を抽出する手法を設計する必要がある．特に，熱ひずみ以外の誤差源を誤ってモデルに取り込むと補正が不安定化するため，残差分解とモデル更新則の設計が重要である．
