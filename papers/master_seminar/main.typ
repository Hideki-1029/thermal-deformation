...
#figure(
  image("figure/image_modify_scan_area.png", width: 95%),
...
評価指標は，捕捉成功率，平均捕捉時間，初期指向誤差，必要scan areaとする．粗捕捉における探索コストは不確定領域の面積に対応するため，補正後残差の分散または上限値を用いて必要scan areaを評価する．予備的なmock thermal LOS biasを用いた数値実験では，補正なしに対してfeedforward補正を加えることで，平均初期誤差が $93.9$ µradから $49.5$ µradに低下し，平均捕捉時間が $2.43$ sから $0.74$ sに低下する傾向が確認されている．今後は，この仮想モデルをThermal Desktop / Femapに基づく参照時系列へ置き換え，モデル化誤差を含めた評価を行う．
